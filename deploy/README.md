...
This directory contains files for deploying Sub2API on Linux servers, Apple-silicon Macs, and Kubernetes clusters.
...
| `helm/sub2api/` | Helm chart for Kubernetes deployment. `fleet.yaml` lives alongside `Chart.yaml` so Rancher Continuous Delivery (Fleet) can consume `deploy/helm/sub2api` directly as a GitRepo `path` |
